...
- 기사에 없는 내용은 쓰지 않는다.
- 카드뉴스와 캡션에는 `기사상`, `보도된`, `보도 기준`, `기사에서 확인된 내용`처럼 출처를 반복 설명하는 표현을 사용하지 않는다. 출처는 하단 표기로 충분하므로 `목표`, `기준`, `확인 내용`처럼 바로 쓴다.
...
- 이미지 사용 시 텍스트 가독성을 위한 다크 오버레이를 적용하되, 건물·조감도·지도 등 배경의 형태와 핵심 요소를 육안으로 알아볼 수 있는 밝기를 유지한다.
- 기본 사진 레이어는 약 94~98% 불투명도로 표시하고, 텍스트 쪽 다크 오버레이는 약 53~66%, 반대쪽은 약 34% 수준으로 적용한다. 배경이 사실상 보이지 않을 정도로 과도하게 어둡게 처리하지 않는다.
...
2026-09-05 최종 반영: 목록 표시 순서는 주택공급(공급정책·청약·분양) → 철도·교통 → 개발(재개발·재건축·신도시·택지)이다. 각 분야 안에서는 중요도 점수순으로 정렬한다. 광역철도, 지하철·도시철도 연장/신설, GTX, 철도 예타·기본계획·역세권을 별도로 검색한다. 철도 사업 후보는 전체 30건 중 최대 5건을 우선 확보한 뒤 위 분야 순서로 배치한다. 관련 기사가 적으면 남은 자리는 일반 후보로 채운다. 개통 기념 행사·여행 공모·사고 등은 철도 우선 확보에서 제외한다. 사용자에게 이미 제시한 당일 기사 번호는 재수집으로 변경하지 않는다.
