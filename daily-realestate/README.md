...
- 기사에 사용할 수 있는 건물 전경·조감도·지도·도면 배경 파일이 없으면 `gpt-image-2`로 세로형 보조 배경 1장을 자동 생성한다.
- 생성 이미지에는 인물·글자·숫자·로고·표지판·문서·차트를 넣지 않으며, 특정 사업의 실제 조감도로 오인되지 않는 일반적인 편집용 이미지로 만든다.
- 기존 `article-XX-bg` 파일이 있으면 이미지 생성 API를 호출하지 않는다.
- `OPENAI_API_KEY`가 없거나 생성 API가 실패하면 게시 작업을 중단하지 않고 기존 네이비 배경으로 안전하게 전환하며 `image-generation-report.json`에 사유를 기록한다.
