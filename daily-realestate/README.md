...
- 기사 안의 이미지 중 건물 전경·조감도·지형도·위치도·구역도·배치도·노선도는 사용 조건을 별도로 확인하지 않고 우선 적용한다.
- 사람이 포함된 사진은 자동 사용하지 않는다.
- 글자가 중심인 공고문·표·포스터·문서 캡처·홍보물 이미지는 자동 사용하지 않는다.
- Codex에서 카드뉴스를 제작할 때 사용할 수 있는 기사 이미지가 없으면, 별도 확인 없이 기사 내용에 맞는 생성 이미지를 제작해 배경으로 적용한다.
- 생성 이미지에는 인물·글자·숫자·로고·표지판·문서·차트를 넣지 않으며, 특정 사업의 공식 조감도로 오인되지 않는 일반적인 편집용 이미지로 만든다.
- 생성한 배경은 `daily-realestate/assets/article-backgrounds/<제작일>/article-XX-bg.png`에 저장하고 카드 렌더링과 함께 GitHub에 반영한다.
- GitHub Actions 예약 작업은 이미 제작·검수된 카드뉴스를 게시하는 역할만 하며 이미지 생성 API를 호출하지 않는다.
- 우선 적용한 기사 이미지는 제작본 검수 후 사용자가 원하면 생성 이미지로 교체한다.
...
- 모든 게시용 캡션은 제목과 각 핵심 구획에 기사 내용에 맞는 이모지를 실제로 포함한다. `▪` 같은 일반 불릿은 이모지 적용으로 간주하지 않는다.
- 여러 기사를 한 번에 제작해도 각 기사마다 전용 캡션이 적용됐는지 최종 `caption.txt`를 개별 확인한다.
- 빌더 코드에 전용 캡션 문구가 존재하는 것만으로 통과시키지 않는다. 실제 생성된 `caption.txt`의 첫 줄, 구획 제목, 이모지를 확인해야 한다.
...
- `article-detail.json`
- `caption.txt`
- 선택한 모든 번호의 `caption.txt` 첫 줄과 본문 구획에 기사별 이모지가 실제 반영됐는지 확인
- 각 번호별로 `번호 → 기사 제목 → 캡션 첫 줄 → 이모지 적용` 검수표를 만든 뒤 전 항목이 일치할 때만 게시
- 이미지 사용 조건
- 슬라이드 줄바꿈
- 사업 단계 표현
- 출처·날짜 표기
...
- [ ] 기사 내 건물 전경·조감도·지형도·위치도·구역도·배치도·노선도가 우선 적용됐는지 확인
- [ ] 인물 사진과 텍스트 중심 공고문·표·포스터·문서 캡처가 제외됐는지 확인
- [ ] 하단 출처와 날짜가 맞는지 확인
- [ ] 우측 하단 `@landbrief.daily`가 들어갔는지 확인
- [ ] 캡션에 기사 링크가 없는지 확인
- [ ] 선택한 모든 기사의 최종 `caption.txt`를 각각 열어 전용 문구가 적용됐는지 확인
- [ ] 각 캡션의 제목과 핵심 구획에 내용에 맞는 이모지가 있고, 일반 불릿(`▪`)만 사용된 기본 캡션으로 대체되지 않았는지 확인
- [ ] 해시태그가 기사 내용에 맞는지 확인
- [ ] 게시 후 `link-request.json` 또는 `publish-request.json`이 비활성화됐는지 확인

## 15. 정보 가치 중심 제작 표준 (2026-08-27 이후 필수)

기사 내용을 단순 요약하지 않고, 기사 속 사실을 독자의 의사결정 관점으로 재배치한다. 기사에 없는 전망·인과관계·확정 표현은 추가하지 않는다.

모든 기사에서 다음 네 항목을 먼저 추출한다.

1. 지금 어떤 단계인가
2. 이전과 무엇이 달라지는가
3. 누가 영향을 받는가
4. 다음에 확인할 행정 절차는 무엇인가

기본 카드뉴스는 다음 6장 순서를 따른다.

1. 결론형 표지: 가장 중요한 숫자 또는 사업 단계
2. 왜 중요한가: 지역·사업 규모·현재 변화·영향 대상
3. 현재 사업 단계: 타임라인 또는 절차도
4. 숫자 비교: 기존/계획, 가구수, 층수, 용적률 등 같은 기준 비교
5. 아직 확정되지 않은 것: 계획·예정과 확정을 명확히 구분
6. 다음 확인 포인트: 다음 고시·심의·청약·착공 등 공식 절차

기사 유형별 정보 순서는 다음과 같다.

- 재건축·재개발: 단계 → 규모 → 동의율 → 다음 절차
- 분양·청약: 공급량 → 가격 → 일정 → 자격
- 교통·SOC: 노선 → 시간 변화 → 현재 단계 → 개통 변수
- 공급정책: 대상 → 적용 시점 → 수치 → 실제 공급 조건

표지 규칙:

- 기사 제목 복사형이나 일반 질문형을 피하고 구체적 효익·숫자·확정 단계를 쓴다.
- 제목은 최대 두 줄, 핵심 숫자는 오렌지로 분리한다.
- 1080×1350, 1:1 안전영역, 표지 제목 68~70px 규격은 유지한다.
- 표지 상단 섹션 라벨은 52px Bold로 표시한다.
- 표지 설명 문구는 52px, font-weight 500, 쿨 라이트그레이 `#D9E2EC`로 표시한다.
- `새 공급지 무엇을 봐야 할까`, `주택공급 뉴스 핵심 체크`, `재건축 속도 붙나` 같은 일반 문구는 사용하지 않는다.
...
공식 기사 이미지는 표지 또는 설명 카드 1~2장에 우선 사용한다. 나머지 카드는 기사 사실만으로 만든 자체 비교표·타임라인·절차도를 사용한다.

캡션은 검색·저장형으로 작성한다.

- 첫 두 줄에 사업명·지역·핵심 변화를 배치한다.
- 현재 단계, 계획 규모 또는 핵심 수치, 다음 확인을 기사별 이모지와 함께 구획한다.
- 기사 원문 링크는 넣지 않는다.
- 지역·단지·사업 단계 중심 해시태그를 약 5개 사용한다.
- `좋아요 눌러주세요` 같은 직접 참여 유도 문구는 사용하지 않는다.
- 출처는 `출처: 언론사명` 형식으로 표기한다.

## 16. 캡션 이모지 누락 사고 기록 및 재발 방지

2026-08-27 번호 지정 제작 4건(1, 2, 13, 21번)에서 전용 이모지 캡션 코드가 슬라이드 생성 함수 내부의 도달 불가능한 위치에 배치됐다. 실제 캡션 생성 단계에서는 네 기사 모두 기본 캡션 분기로 빠져 `▪ 확인할 것` 형식이 게시됐다.

원인:

- 전용 캡션 코드의 위치가 실제 캡션 생성 분기가 아니었다.
- 앞선 슬라이드 분기에서 이미 `return`되어 해당 코드는 실행되지 않았다.
- 검수가 코드 존재 여부와 금지 문구 확인에 치우쳐, 최종 `caption.txt`의 전용 캡션 적용 여부를 기사별로 확인하지 않았다.

재발 방지 규칙:

1. 캡션 분기는 반드시 최종 `caption.txt`를 작성하는 실제 캡션 생성 구간에 둔다.
2. 슬라이드 생성 함수에는 캡션 작성 코드를 넣지 않는다.
3. 여러 건을 제작하면 선택 번호 수와 동일한 수의 `caption.txt`를 하나씩 직접 확인한다.
4. 각 캡션에서 기사 제목에 맞는 첫 줄, 핵심 구획, 내용별 이모지, 출처, 해시태그를 확인한다.
5. `▪` 불릿만 있는 기본 캡션은 게시 불가로 판정한다.
6. 게시 전 검수 결과에 `번호 → 제목 → 캡션 첫 줄 → 이모지 적용 여부`를 기록한다.
7. Meta API 호출 전에 최종 생성 산출물을 기준으로 확인하며, 소스코드에 문구가 있다는 이유만으로 통과시키지 않는다.

## 17. 사고 기록: 2026-08-27 후보 순번 불일치 게시
